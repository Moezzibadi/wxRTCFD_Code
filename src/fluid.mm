    ...
    @autoreleasepool {
        ...
        if (mtlDevice) {
            ...
            NSString* shaderSource = @"#include <metal_stdlib>\n"
                                     ...
                                     "    uint2 id [[thread_position_in_grid]])\n"
                                     "{\n"
                                     "    if (id.x >= (uint)numX || id.y >= (uint)numY) return;\n"
                                     "    int n = numY; int idx = id.x * n + id.y; float h2 = 0.5f * h;\n"
                                     "    newU[idx] = u[idx]; newV[idx] = v[idx];\n"
                                     "    if (id.x >= 1 && id.y >= 1) {\n"
                                     "        if (id.x < (uint)numX && id.y < (uint)numY - 1 && s[idx] != 0.0f && s[(id.x - 1) * n + id.y] != 0.0f) {\n"
                                     ...
                                     "            x -= dt * _u; y -= dt * _v;\n"
                                     "            newU[idx] = sampleField(u, x, y, numX, numY, h, 0.0f, h2);\n"
                                     "        }\n"
                                     "        if (id.x < (uint)numX - 1 && id.y < (uint)numY && s[idx] != 0.0f && s[id.x * n + id.y - 1] != 0.0f) {\n"
                                     ...
                                     "            float _v = v[idx];\n"
                                     "            x -= dt * _u; y -= dt * _v;\n"
                                     "            newV[idx] = sampleField(v, x, y, numX, numY, h, h2, 0.0f);\n"
                                     "        }\n"
                                     "    }\n"
                                     "}\n"
                                     "kernel void advectTracer(\n"
                                     "    device const float* u [[buffer(0)]], device const float* v [[buffer(1)]],\n"
                                     "    device const float* m [[buffer(2)]], device float* newM [[buffer(3)]],\n"
                                     ...
                                     "{\n"
                                     "    if (id.x >= (uint)numX || id.y >= (uint)numY) return;\n"
                                     "    int n = numY; int idx = id.x * n + id.y;\n"
                                     "    if (id.x <= 1) {\n" // Inlet Boundary Logic
                                     "        float pipeH = 0.1f * (float)numY;\n"
                                     "        int minJ = (int)floor(0.5f * (float)numY - 0.5f * pipeH);\n"
                                     "        int maxJ = (int)floor(0.5f * (float)numY + 0.5f * pipeH);\n"
                                     "        newM[idx] = (id.y >= (uint)minJ && id.y < (uint)maxJ) ? 0.0f : 1.0f;\n"
                                     "        return;\n"
                                     "    }\n"
                                     "    if (id.x < 1 || id.y < 1 || id.x >= (uint)numX - 1 || id.y >= (uint)numY - 1 || s[idx] == 0.0f) {\n"
                                     "        newM[idx] = m[idx]; return;\n"
                                     "    }\n"
                                     "    float h2 = 0.5f * h;\n"
                                     "    float _u = (u[idx] + u[(id.x + 1) * n + id.y]) * 0.5f;\n"
                                     "    float _v = (v[idx] + v[id.x * n + id.y + 1]) * 0.5f;\n"
                                     "    float x = id.x * h + h2 - dt * _u, y = id.y * h + h2 - dt * _v;\n"
                                     "    newM[idx] = sampleField(m, x, y, numX, numY, h, h2, h2);\n"
                                     "}\n"
